...
def load_calories():
    saved_files_dir = os.listdir(
        "/Users/angushirst/Northcoders_followup/calorie-calculator/saved_calorie_data"
    )
    os.system("clear")
    time.sleep(0.25)
    print("\nLOAD CALORIES\n-------------")
    ...
    if saved_file_count == 0:
        ...
        if f"{user_load_input}_calories.csv" in saved_files_dir:
            loaded_data = []
            df = pd.read_csv(
                f"/Users/angushirst/Northcoders_followup/calorie-calculator/saved_calorie_data/{user_load_input}_calories.csv"
            )
            load_count = 0
